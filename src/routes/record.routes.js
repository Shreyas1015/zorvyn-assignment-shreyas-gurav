...
router.get('/', authorize('ANALYST', 'ADMIN'), validateQuery(recordQuerySchema), recordController.list);
router.get('/:id', authorize('ANALYST', 'ADMIN'), validateParams(uuidParam), recordController.getById);
router.post('/', authorize('ADMIN'), validate(createRecordSchema), recordController.create);
router.patch('/:id', authorize('ADMIN'), validateParams(uuidParam), validate(updateRecordSchema), recordController.update);
